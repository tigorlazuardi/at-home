		...
		<div className={prop.className}>
			<div className="flex flex-wrap gap-x-4 gap-y-8 justify-around">
				{data.map((k) => (
					<div
						key={k.key}
						className="divide-y-4 divide-black divide-double dark:divide-white basis-1/5 grow sm-max:basis-1/2"
					>
						<div className="pb-2 pl-2 font-bold">{k.key}</div>
						<div className="pt-2 pl-2">{k.value}</div>
					</div>
				))}
			</div>
		</div>
